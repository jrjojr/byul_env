...
이 저장소는 BYUL 프로젝트다. 작업을 시작할 때는 `external/rules_env` Git
submodule의 독립 `rules_env`가 소유하는 다음 Emacs Org 중앙 규칙을 순서대로 읽고
따른다.

```text
external/rules_env/common/init-rules.org
external/rules_env/common/project-bootstrap-rules.org
external/rules_env/projects/byul/byul-init-rules.org
```
