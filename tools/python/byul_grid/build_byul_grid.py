...
DEFAULT_VENV = PYTHON_ROOT / ".venv"
REQUIREMENTS = PYTHON_ROOT / "requirements-package.txt"
BASE_REQUIREMENTS = PYTHON_ROOT / "requirements.txt"
STAMP_NAME = ".requirements-package.sha256"
WRAPPER_ABI_GENERATOR = ROOT.parent / "byul_wrapper" / "generate_wrapper_abi.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="shared Python tools virtual environment (default: ../.venv)",
    )
    parser.add_argument(
        "--force-dependencies",
        action="store_true",
...
def requirements_digest() -> str:
    ...
    for path in (REQUIREMENTS, BASE_REQUIREMENTS):
...
def main() -> int:
    ...
    try:
        # Keep CFFI declarations synchronized with the current BYUL public
        # headers before freezing the application.
        run([sys.executable, str(WRAPPER_ABI_GENERATOR)])
        clean_legacy_layout()
