...
def test_navgrid_declaration_definition_export_wrapper_intersection():
    ...
    wrapper_declarations = set(
        re.findall(r"\b(navgrid_\w+|is_coord_blocked_navgrid)\s*\(", cdef)
    )
    missing_exports = {name for name in declarations if not hasattr(C, name)}
