...
if [ -n "${BYUL_PYTHON:-}" ]; then
    exec "$BYUL_PYTHON" "$SCRIPT_DIR/header_refactor_build_snapshot.py" "$@"
fi
if [ -x "$SCRIPT_DIR/python/.venv/bin/python" ]; then
    exec "$SCRIPT_DIR/python/.venv/bin/python" \
        "$SCRIPT_DIR/header_refactor_build_snapshot.py" "$@"
fi
if command -v python3 >/dev/null 2>&1; then
    exec python3 "$SCRIPT_DIR/header_refactor_build_snapshot.py" "$@"
fi
