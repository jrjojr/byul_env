...
이 문서는 `tools/byul_grid`를 cx_Freeze로 독립 실행형 애플리케이션으로 만드는
...
```text
tools/byul_grid/build_byul_grid.py
tools/byul_grid/build_byul_grid.bat
tools/byul_grid/build_byul_grid.sh
tools/byul_grid/setup_freeze.py
tools/byul_grid/requirements-package.txt
tools/byul_grid/byul_grid/config.py
tools/byul_grid/byul_grid/wrapper/modules/ffi_core.py
```
...
```text
tools/byul_grid/.venv-package
```

`build_byul_grid.py`는 다음 순서로 환경을 준비합니다.

1. `.venv-package`가 없으면 현재 Python으로 생성한다.
...
![cx_Freeze config 모듈 누락 오류](../화면%20캡처%202026-07-12%20003730.png)
...
![cx_Freeze pycparser 순환 import 오류](../화면%20캡처%202026-07-12%20004232.png)
...
![cx_Freeze NumPy docstring 초기화 오류](../스크린샷%202026-07-12%20005403.png)
...
패키징 의존성 파일의 해시가 변경되므로 다음 빌드에서 `.venv-package`의
...
```text
Windows: build_win_msvc_release/bin/Release/byul.dll
...
```text
tools/byul_grid/build/windows-x86_64/byul_grid/
├── byul_grid.exe
├── byul.dll
├── python313.dll
├── lib/
...
```text
tools/byul_grid/build/linux-x86_64/byul_grid/
├── byul_grid
├── lib/
│   ├── libbyul.so
...
```text
tools/byul_grid/build/macos-arm64/byul_grid/
├── byul_grid
├── lib/
│   └── libbyul.dylib
├── share/
...
```text
tools/byul_grid/assets/icons/byul_grid.ico
```
...
```powershell
cmake --build build_win_msvc_release `
    --target grid_zip `
    --config Release
```
...
`tools/byul_grid/build` 산출물과 충돌하지 않습니다. cx_Freeze는 호스트 OS용
