...
TEST_CASE("square, clamp, sign") {
    CHECK(float_square(3.0f) == doctest::Approx(9.0f));
    CHECK(float_clamp(5.0f, 1.0f, 4.0f) == doctest::Approx(4.0f));
    CHECK(float_clamp(-2.0f, -1.0f, 1.0f) == doctest::Approx(-1.0f));
    CHECK(float_sign(3.0f) == doctest::Approx(1.0f));
    CHECK(float_sign(-3.0f) == doctest::Approx(-1.0f));
    CHECK(float_sign(0.0f) == doctest::Approx(0.0f));
}

TEST_CASE("deg2rad, rad2deg") {
    CHECK(float_deg2rad(180.0f) == doctest::Approx(3.1415926f).epsilon(0.001f));
    CHECK(float_rad2deg(3.1415926f) == doctest::Approx(180.0f).epsilon(0.001f));
}

TEST_CASE("lerp and inverse lerp") {
    CHECK(float_lerp(0.0f, 10.0f, 0.5f) == doctest::Approx(5.0f));
    CHECK(float_inv_lerp(0.0f, 10.0f, 5.0f) == doctest::Approx(0.5f));
}

TEST_CASE("remap and clamp01") {
    CHECK(float_remap(0.0f, 10.0f, 100.0f, 200.0f, 5.0f) == doctest::Approx(150.0f));
    CHECK(float_clamp01(1.2f) == doctest::Approx(1.0f));
    CHECK(float_clamp01(-0.2f) == doctest::Approx(0.0f));
    CHECK(float_clamp01(0.5f) == doctest::Approx(0.5f));
}

TEST_CASE("smoothstep") {
    CHECK(float_smoothstep(0.0f, 1.0f, 0.0f) == doctest::Approx(0.0f));
    CHECK(float_smoothstep(0.0f, 1.0f, 0.5f) == doctest::Approx(0.5f).epsilon(0.1f));
    CHECK(float_smoothstep(0.0f, 1.0f, 1.0f) == doctest::Approx(1.0f));
}

TEST_CASE("float_equal_tol") {
...
TEST_CASE("float_equal_tol_all") {
    float a = 1.0f;

    // tol_pos = 0.002, tol_neg = 0.001
    CHECK(float_equal_tol_all(a, 1.002f, 0.002f, 0.001f) == true);
    CHECK(float_equal_tol_all(a, 1.003f, 0.002f, 0.001f) == false);
    CHECK(float_equal_tol_all(a, 0.999f, 0.002f, 0.001f) == true);
    CHECK(float_equal_tol_all(a, 0.998f, 0.002f, 0.001f) == false);

    CHECK(float_equal_tol_all(a, 1.001f, -0.002f, -0.001f) == true);
}

TEST_CASE("float_equal_tol_all 1") {
    float a = 1.0f;
