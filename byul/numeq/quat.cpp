...
quat_t* quat_new_axis_angle(const vec3_t* axis, float radians) {
    ...
    quat_t* r = new quat_t();
    from_eigen(r, Quaternionf(AngleAxisf(radians, vec3_to_eigen(axis).normalized())));
    return r;
}
...
void quat_from_euler(quat_t* out, float rx, float ry, float rz, euler_order_t order) {
    if (!out) return;
...
void quat_to_euler(const quat_t* r, float* x, float* y, float* z, euler_order_t order) {
    if (!r || !x || !y || !z) return;
    Matrix3f m = to_eigen(r).toRotationMatrix();
    switch (order) {
