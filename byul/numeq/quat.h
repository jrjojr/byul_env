...
typedef enum euler_order_t {
    EULER_ORDER_ZYX, ///< Z → Y → X (Roll → Yaw → Pitch), 일반적으로 가장 많이 사용됨
    EULER_ORDER_XYZ,
    EULER_ORDER_XZY,
    EULER_ORDER_YXZ,
    EULER_ORDER_YZX,
...
BYUL_API void quat_from_axis_angle(quat_t* out, const vec3_t* axis, float radians);

// -----------------------------
// quat_from_axis_deg
// -----------------------------
BYUL_API void quat_from_axis_deg(quat_t* out, const vec3_t* axis, float degrees);
...
BYUL_API void quat_rotate_vector(vec3_t* out, const quat_t* r, const vec3_t* v);
...
BYUL_API void quat_lerp(quat_t* out, const quat_t* a, const quat_t* b, float t);

/** @brief 두 회전 구면 선형 보간 (정확하고 부드러움) */
BYUL_API void quat_slerp(quat_t* out, const quat_t* a, const quat_t* b, float t);
