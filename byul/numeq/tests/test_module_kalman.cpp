...
TEST_CASE("Scalar Kalman: Initialization and update") {
    kalman_filter_t kf;
    kalman_reset(&kf, 0.0f, 1.0f, 0.01f, 1.0f);  // x=0, p=1

    CHECK(kf.x == doctest::Approx(0.0f));
    CHECK(kf.p == doctest::Approx(1.0f));

    float m1 = 1.0f;
    float x1 = kalman_update(&kf, m1);  // 측정값 = 1.0
    CHECK(x1 > 0.0f);
...
TEST_CASE("Scalar Kalman: Predict should increase uncertainty") {
    kalman_filter_t kf;
    kalman_reset(&kf, 5.0f, 1.0f, 0.1f, 1.0f);
...
TEST_CASE("Vec3 Kalman: Initialization and basic prediction") {
    kalman_filter_vec3_t kf;
    vec3_t init_pos = {0.0f, 0.0f, 0.0f};
    vec3_t init_vel = {1.0f, 2.0f, 3.0f};

    kalman_vec3_reset(&kf, &init_pos, &init_vel, 0.1f, 1.0f, 1.0f);  // dt = 1.0

    kalman_vec3_predict(&kf);  // 위치 += 속도 * dt
...
TEST_CASE("Vec3 Kalman: Update brings prediction closer to measurement") {
    kalman_filter_vec3_t kf;
    vec3_t init_pos = {0.0f, 0.0f, 0.0f};
    vec3_t init_vel = {1.0f, 0.0f, 0.0f};
    kalman_vec3_reset(&kf, &init_pos, &init_vel, 0.1f, 1.0f, 1.0f);

    kalman_vec3_predict(&kf);

    vec3_t measured = {2.0f, 0.0f, 0.0f};  // 측정값이 더 멀리 있음
    kalman_vec3_update(&kf, &measured);
...
TEST_CASE("Vec3 Kalman: Project future position") {
    ...
    kalman_vec3_reset(&kf, &pos, &vel, 0.1f, 1.0f, 1.0f);
...
TEST_CASE("Scalar Kalman: Covariance P decreases after update") {
    kalman_filter_t kf;
    kalman_reset(&kf, 0.0f, 10.0f, 0.1f, 1.0f);  // 큰 초기 오차 공분산
...
TEST_CASE("Scalar Kalman: Random noisy measurements converge to true value") {
    kalman_filter_t kf;
    kalman_reset(&kf, 0.0f, 1.0f, 0.01f, 0.5f);
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        last_estimate = kalman_update(&kf, noisy_measured);
    }
