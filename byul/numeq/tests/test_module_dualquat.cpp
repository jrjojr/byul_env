...
TEST_CASE("dualquat_inverse reverses transformation") {
    ...
    dualquat_from_quat_vec(&dq, &qid, &pos);
    dualquat_inverse(&inv, &dq);
    dualquat_mul(&res, &dq, &inv);

    vec3_t zero = {0}, result;
    dualquat_to_quat_vec(&res, &qid, &result);
...
TEST_CASE("dualquat_blend_weighted produces midpoint") {
    ...
    quat_t qid; quat_identity(&qid);
    dualquat_from_quat_vec(&a, &qid, &va);
    dualquat_from_quat_vec(&b, &qid, &vb);
...
TEST_CASE("dualquat_transform_point: rotation and translation") {
    // 회전: Y축 90도, 이동: (5, 0, 0)
    vec3_t axis = {0, 1, 0};
    quat_t rot;
    quat_from_axis_deg(&rot, &axis, 90.0f);

    vec3_t trans = {5.0f, 0.0f, 0.0f};
    dualquat_t dq;
    dualquat_from_quat_vec(&dq, &rot, &trans);

    vec3_t point = {1.0f, 0.0f, 0.0f};
    dualquat_transform_point(&dq, &point);
