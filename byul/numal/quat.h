...
BYUL_API void quat_init_full(quat_t* out, float w, float x, float y, float z);

/** @brief 축+라디안 회전 생성 (동적 할당됨) */
BYUL_API void quat_init_axis_angle(
    quat_t* out, const vec3_t* axis, float radians);

/** @brief 축+도(degree) 회전 생성 (동적 할당됨) */
BYUL_API void quat_init_axis_deg(
    quat_t* out, const vec3_t* axis, float degrees);
...
BYUL_API void quat_init_angular_velocity(
...
BYUL_API void quat_to_axis_angle(
    const quat_t* q, vec3_t* out_axis, float* out_radians);
