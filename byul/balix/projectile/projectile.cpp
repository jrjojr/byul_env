...
void projectile_copy(projectile_t* out, const projectile_t* src) {
    if (!out || !src) return;
    *out = *src;
}
...
void shell_copy(shell_t* out, const shell_t* src) {
    if (!out || !src) return;
    *out = *src;
}
...
void missile_copy(missile_t* out, const missile_t* src) {
    if (!out || !src) return;
    *out = *src;
}
...
void projectile_predictor_copy(projectile_predictor_t* out,
...
void missile_predictor_copy(missile_predictor_t* out,
...
projectile_result_t* projectile_result_clone(const projectile_result_t* src) {
    if (!src) return NULL;
    ...
    copy->trajectory = trajectory_clone(src->trajectory);
    return copy;
}
