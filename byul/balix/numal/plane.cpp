...
// n = normalize((b-a) x (c-a)), d = -dot(n, a)
...
// v' = v*cos + (k x v)*sin + k*(k.v)*(1-cos), k is unit
...
// Origin rotation: n' = R n, d is invariant under rotation about the origin.
    ...
    plane_t* p, const vec3_t* axis_unit, float angle_rad)
{
    ...
    // An orthonormal rotation preserves length, but renormalize to curb numerical drift.
    (void)safe_normalize(&p->normal_unit);
    // d stays unchanged for origin-centered rotations.
    return true;
}
    ...
    plane_t* p, const vec3_t* axis_unit, float angle_rad, const vec3_t* pivot)
{
    ...
    // 2) Choose a point x0 on the plane (closest to the origin)
    vec3_t x0;
    plane_anchor_point(p, &x0);  // x0 = -d * n

    // 3) Rotate that point around the pivot
