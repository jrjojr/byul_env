...
TEST_CASE("coord: create and free") {
    coord_t* c = coord_create_full(10, 20);
    REQUIRE(c != nullptr);
...
TEST_CASE("coord: degree basic") {
    ...
    coord_t* c = coord_create_full(0, 1);
    coord_t* d = coord_create_full(-1, 0);
    coord_t* e = coord_create_full(0, -1);
    ...
    coord_destroy(a); coord_destroy(b); coord_destroy(c); coord_destroy(d); coord_destroy(e);
}

TEST_CASE("coord: degree diagonal") {
    coord_t* a = coord_create_full(0, 0);
    coord_t* b = coord_create_full(1, 1);
    coord_t* c = coord_create_full(-1, 1);
    coord_t* d = coord_create_full(-1, -1);
    coord_t* e = coord_create_full(1, -1);
