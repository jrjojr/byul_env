...
TEST_CASE("dstar_lite_pqueue multiple pushes with same key") {
    auto* q = dstar_lite_pqueue_new();

    dstar_lite_key_t* k = dstar_lite_key_new_full(1.5, 2.5);
    ...
    coord_free(c2);
    coord_free(c3);
    dstar_lite_key_free(k);
    dstar_lite_pqueue_free(q);
}

TEST_CASE("dstar_lite_pqueue interleaved same key and different key") {
    auto* q = dstar_lite_pqueue_new();

    dstar_lite_key_t* k1 = dstar_lite_key_new_full(1.0, 2.0);
    dstar_lite_key_t* k2 = dstar_lite_key_new_full(0.5, 1.5); // 더 우선순위 높음

    coord_t* a1 = coord_new_full(10, 10);
    coord_t* a2 = coord_new_full(11, 11);
    coord_t* b1 = coord_new_full(5, 5);
    ...
    coord_free(a2);
    coord_free(b1);
    dstar_lite_key_free(k1);
    dstar_lite_key_free(k2);
    dstar_lite_pqueue_free(q);
}
