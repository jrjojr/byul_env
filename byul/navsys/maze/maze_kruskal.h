...
/**
 * @file maze_kruskal.h
 * @brief Declares the deterministic randomized Kruskal Maze public C ABI.
 */
...
/**
 * @brief Generates a deterministic perfect Maze with randomized Kruskal.
 *
 * The logical grid's unique horizontal and vertical edges are shuffled with
 * the common seeded PCG stream. Edges joining distinct components are opened
 * until the spanning tree is complete. Equal extents and seeds produce an
 * equal raster. The logical passage graph is connected and acyclic. This is
 * randomized edge ordering; it makes no minimum-weight or uniform-spanning-
 * tree guarantee. On failure, *out_maze is NULL.
 *
 ...
 * @complexity O(E alpha(V)) time and O(width*height + V + E) storage, where
 * V=(width/2)*(height/2) and E is the logical grid's unique edge count.
 */
...
/**
 * @brief Generates a randomized Kruskal Maze with legacy process-local seeding.
 *
 * Use the checked API for deterministic replay, cancellation, and resource
 * budgets.
 *
 * @deprecated Use byul_maze_generate_randomized_kruskal. Removal requires ABI 2 or later.
 ...
 */
BYUL_DEPRECATED(
    "Use byul_maze_generate_randomized_kruskal; removal requires ABI 2 or later.")
