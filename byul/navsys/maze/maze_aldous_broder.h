...
/**
 * @brief Generates a replayable uniform-spanning-tree Maze with Aldous-Broder.
 *
 * A simple random walk opens an edge only on the destination cell's first
 * visit. The same extent and seed produce the same raster. On failure,
 * `*out_maze` is NULL and no partial Maze is published.
 *
 ...
 * @complexity Expected cover-time complexity with unbounded random-walk
 * variance, plus O(width*height) temporary and output storage.
 */
 ...
 * @deprecated Use byul_maze_generate_aldous_broder. Removal requires ABI 2 or later.
 ...
 */
BYUL_DEPRECATED(
    "Use byul_maze_generate_aldous_broder; removal requires ABI 2 or later.")
